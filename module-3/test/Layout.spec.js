...
describe('Layout Class', () => {
    ...
    describe('Get', () => {
        ...
        it('should have method to retrieve nested children element by name', () => {
            const page = new Layout('Body', 'http://epam.com', {css: 'body'});
            const child = new Element('Title', {css: 'h1'});
            const firstNestedChild = new Element('First Subtitle', {css: 'h2'});
            const secondNestedChild = new Element('Second Subtitle', {css: 'h2'});
            firstNestedChild.addChildren(secondNestedChild);
            child.addChildren(firstNestedChild);
            page.addChildren(child);

            const pElement = page.get('Second Subtitle');

            expect(pElement).to.be.instanceOf(ElementFinder);
            expect(pElement.locator().css).to.equal('h2');
        });
